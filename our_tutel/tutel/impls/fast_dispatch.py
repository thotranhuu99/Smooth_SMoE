...
# Our
def extract_critical_smooth(scores, logits_w_noise, bd_loss, top_k, loss_fn=losses.gshard_loss, capacity_factor=1.0, batch_prioritized_routing=False, normalize_gate=True, alignment=1, group=None, inequivalent_tokens=False):
    num_global_experts = int(scores.size(1))
    top_k, top_k_original = min(top_k, num_global_experts), top_k
    topk_indices = torch.topk(scores, top_k, dim=1).indices
    ...
    # Xác định số lượng experts thực sự cần thiết cho mỗi token
    gate_masks = [gate > 0 for gate in gates_s]

    # Tìm số lượng experts tối đa thực sự cần thiết - ĐẢM BẢO ĐỒNG BỘ GIỮA CÁC PROCESSES
    actual_top_k = top_k
    for k in range(top_k - 1, -1, -1):
        if torch.any(gate_masks[k]):
            actual_top_k = k
            break

    # Đồng bộ actual_top_k giữa tất cả processes
    actual_top_k_tensor = torch.tensor(actual_top_k, device=scores.device)
    actual_top_k = int(simple_all_reduce(actual_top_k_tensor, group=group, op=torch.distributed.ReduceOp.MAX))

    # Cắt bỏ các experts không cần thiết
    if actual_top_k < top_k:
        indices_s = indices_s[:actual_top_k]
        masks_se = masks_se[:actual_top_k]
        gates_s = gates_s[:actual_top_k]
        top_k = actual_top_k

    l_loss = loss_fn(logits_w_noise, topk_indices) if loss_fn is not None else None
    ...
    locations2.topk_ids = topk_indices
    return (num_global_experts, indices_s, locations_s, gates_s, capacity, locations2), l_loss + bd_loss
