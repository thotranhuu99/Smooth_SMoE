...
class GMOE(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        gate = hparams['router'] if 'router' in hparams else 'cosine_top'
        num_experts = hparams['num_experts'] if 'num_experts' in hparams else 6
        topk = hparams['topk'] if 'topk' in hparams else 1
        self.model = vision_transformer.deit_small_patch16_224(pretrained=True, num_classes=num_classes, moe_layers=['F'] * 8 + ['S', 'F'] * 2, mlp_ratio=4., num_experts= num_experts, is_tutel=True, drop_path_rate=0.1, router=gate, topk=topk).cuda()
        topk = self.model.blocks[10].mlp.gates[0].top_k if hasattr(self.model.blocks[10].mlp, 'gates') else 0
        gate = self.model.blocks[10].mlp.gates[0] if hasattr(self.model.blocks[10].mlp, 'gates') else None
        print("____________________ Using GMOE __________________________Router ", gate, " TopK ", topk)
